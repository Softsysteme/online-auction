...
public class PrimeFaces {
	...
	// 1) the current static solution + use
	// Faces/RequestContext#getCurrentInstance each time
	// 2) make PrimeFaces requestScoped and receive Faces/RequestContext only
	// once
	 ...
	 *
	 * @param clientId
	 *            clientId of the target component.
	 */
	...
	public class Dialog {
		 ...
		 *
		 * @param outcome
		 *            the logical outcome used to resolve the navigation case.
		 */
		 ...
		 * @param params
		 *            parameters to send to the view displayed in the dynamic
		 *            dialog.
		 */
		 ...
		 *
		 * @param data
		 *            optional data to pass back to a dialogReturn event.
		 */
		 ...
		 *
		 * @param message
		 *            the {@link FacesMessage} to be displayed.
		 */
		public void showMessageDynamic(FacesMessage message) {
			...
			executeScript("PrimeFaces.showMessageInDialog({severity:\"" + message.getSeverity() + "\",summary:\""
	...
	public Ajax ajax() {
		if (getFacesContext().isProjectStage(ProjectStage.Development) && !isAjaxRequest()) {
			...
			// throw new FacesException("ajax() can only be used in AJAX
			// requests!");
		}
	...
	public class Ajax {
		/**
		 * Add a parameter for ajax oncomplete client side callbacks. Value will
		 * be serialized to json. Currently supported values are primitives,
		 * POJOs, JSONObject and JSONArray.
		 *
		 * @param name
				...
				for (String clientId : clientIds) {
					try {
						...
						LOG.log(Level.WARNING,
								"PrimeFaces.current().ajax().update() called but component can't be resolved!", e);
					}
				}
			}
